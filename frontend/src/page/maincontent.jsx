...
import { 
  ...
  FaMicrochip, FaChartLine, FaMemory, FaHdd,
  FaCheckCircle, FaExclamationTriangle, FaTimesCircle,
  FaTachometerAlt, FaCog, FaInfoCircle, FaQuestionCircle
} from 'react-icons/fa';
...
const hardwarePerformanceDB = {
  cpus: [
    // Low-end
    { name: "Intel Core i3-10100", score: 45, tier: "low", matches: ["i3-10100", "i3"] },
    { name: "AMD Ryzen 3 3100", score: 43, tier: "low", matches: ["ryzen 3", "r3"] },
    { name: "Intel Pentium Gold G6400", score: 38, tier: "low", matches: ["pentium", "g6400"] },
    // Mid-range
    { name: "Intel Core i5-12400F", score: 72, tier: "mid", matches: ["i5-12400", "i5"] },
    { name: "AMD Ryzen 5 5600X", score: 75, tier: "mid", matches: ["ryzen 5", "r5 5600"] },
    { name: "Intel Core i5-11400", score: 68, tier: "mid", matches: ["i5-11400"] },
    // High-end
    { name: "AMD Ryzen 7 7800X3D", score: 95, tier: "high", matches: ["ryzen 7", "r7 7800"] },
    { name: "Intel Core i9-13900K", score: 98, tier: "high", matches: ["i9-13900", "i9"] },
    { name: "AMD Ryzen 9 7950X", score: 96, tier: "high", matches: ["ryzen 9", "r9 7950"] }
  ],
  gpus: [
    // Low-end
    { name: "NVIDIA GTX 1050 Ti", score: 32, tier: "low", matches: ["1050 ti", "gtx 1050"] },
    { name: "NVIDIA GTX 1650", score: 38, tier: "low", matches: ["1650", "gtx 1650"] },
    { name: "AMD Radeon RX 6400", score: 35, tier: "low", matches: ["6400", "rx 6400"] },
    // Mid-range
    { name: "NVIDIA RTX 3060", score: 68, tier: "mid", matches: ["3060", "rtx 3060"] },
    { name: "AMD Radeon RX 6600", score: 65, tier: "mid", matches: ["6600", "rx 6600"] },
    { name: "NVIDIA RTX 3060 Ti", score: 72, tier: "mid", matches: ["3060 ti", "rtx 3060 ti"] },
    // High-end
    { name: "NVIDIA RTX 3090", score: 94, tier: "high", matches: ["3090", "rtx 3090"] },
    { name: "NVIDIA RTX 4090", score: 98, tier: "high", matches: ["4090", "rtx 4090"] },
    { name: "AMD Radeon RX 7900 XTX", score: 90, tier: "high", matches: ["7900 xtx", "rx 7900"] }
  ]
};
...
export default function MainContent() {
  ...
  const parseGameRequirements = (game) => {
    ...
    const yearBonus = game.released ? Math.max(0, (new Date().getFullYear() - new Date(game.released).getFullYear()) * 2) : 0;
    
    // Popular/AAA games have higher requirements
    const popularityBonus = game.playtime ? Math.min(15, game.playtime / 10) : 10;
    
    const requirementScore = Math.min(90, baseScore + yearBonus + popularityBonus);
  ...
  const findHardwareMatch = (hardwareName, type) => {
    const db = type === 'cpu' ? hardwarePerformanceDB.cpus : hardwarePerformanceDB.gpus;
    ...
    if (type === 'cpu') {
      if (searchName.includes('i9') || searchName.includes('ryzen 9') || searchName.includes('7950')) {
        return { score: 95, tier: 'high' };
      } else if (searchName.includes('i7') || searchName.includes('ryzen 7') || searchName.includes('7800')) {
        return { score: 85, tier: 'high' };
      } else if (searchName.includes('i5') || searchName.includes('ryzen 5') || searchName.includes('5600')) {
        return { score: 70, tier: 'mid' };
      } else if (searchName.includes('i3') || searchName.includes('ryzen 3')) {
        return { score: 45, tier: 'low' };
      }
    } else {
      if (searchName.includes('4090') || searchName.includes('3090') || searchName.includes('7900')) {
        return { score: 95, tier: 'high' };
      } else if (searchName.includes('4070') || searchName.includes('3080') || searchName.includes('7800')) {
        return { score: 80, tier: 'high' };
      } else if (searchName.includes('3060') || searchName.includes('4060') || searchName.includes('6600')) {
        return { score: 65, tier: 'mid' };
      } else if (searchName.includes('1650') || searchName.includes('1050') || searchName.includes('6400')) {
  ...
  const calculateCompatibility = (game, specs) => {
    ...
    const ramScore = specs.ram.includes('32GB') ? 85 : 
                    specs.ram.includes('16GB') ? 60 : 
                    specs.ram.includes('8GB') ? 35 : 40;
    
    // Get storage score based on type
    const storageScore = specs.storage.includes('SSD') ? 
                        (specs.storage.includes('NVMe') ? 80 : 60) : 30;
    ...
    const allMeetMin = cpuComparison.meetsMin && gpuComparison.meetsMin && 
                       ramComparison.meetsMin && storageComparison.meetsMin;
    ...
    if (allMeetMin && cpuComparison.meetsRec && gpuComparison.meetsRec) {
      ...
      message = 'Your PC exceeds or meets recommended requirements! You can run this game at high/ultra settings.';
    } else if (allMeetMin) {
      compatibility = 'Good';
      color = 'text-blue-400';
      icon = <FaCheckCircle className="text-blue-400 text-2xl" />;
      message = 'Your PC meets minimum requirements. Game should run at medium settings.';
      ...
      message = 'Your CPU may struggle. Consider lowering settings or upgrading CPU.';
    } else if (cpuComparison.meetsMin && !gpuComparison.meetsMin) {
      compatibility = 'GPU Limited';
      color = 'text-yellow-400';
      icon = <FaExclamationTriangle className="text-yellow-400 text-2xl" />;
      message = 'Your GPU may struggle. Lower graphics settings for better performance.';
    } else {
      compatibility = 'Insufficient';
      color = 'text-rose-400';
      icon = <FaTimesCircle className="text-rose-400 text-2xl" />;
      message = 'Your PC does not meet minimum requirements. Game may not run well.';
    }
    ...
    const componentAnalysis = [
      {
        ...
        status: cpuComparison.meetsRec ? 'Excellent' : cpuComparison.meetsMin ? 'Good' : 'Weak',
        color: cpuComparison.meetsRec ? 'text-emerald-400' : cpuComparison.meetsMin ? 'text-blue-400' : 'text-rose-400'
      },
      {
        name: 'GPU',
        ...
        status: gpuComparison.meetsRec ? 'Excellent' : gpuComparison.meetsMin ? 'Good' : 'Weak',
        color: gpuComparison.meetsRec ? 'text-emerald-400' : gpuComparison.meetsMin ? 'text-blue-400' : 'text-rose-400'
      },
      {
        name: 'RAM',
        ...
        status: ramComparison.meetsRec ? 'Excellent' : ramComparison.meetsMin ? 'Good' : 'Weak',
        color: ramComparison.meetsRec ? 'text-emerald-400' : ramComparison.meetsMin ? 'text-blue-400' : 'text-rose-400'
      },
      {
        name: 'Storage',
        ...
        status: storageComparison.meetsRec ? 'Excellent' : storageComparison.meetsMin ? 'Good' : 'Weak',
        color: storageComparison.meetsRec ? 'text-emerald-400' : storageComparison.meetsMin ? 'text-blue-400' : 'text-rose-400'
      }
    ];
    ...
    if (!cpuComparison.meetsMin) {
      bottlenecks.push(`Your CPU (${specs.cpu}) is below minimum requirements.`);
    }
    if (!gpuComparison.meetsMin) {
      bottlenecks.push(`Your GPU (${specs.gpu}) is below minimum requirements.`);
    }
    if (!ramComparison.meetsMin) {
      bottlenecks.push(`Your RAM is below minimum requirements. Consider upgrading.`);
    }
    if (cpuComparison.meetsMin && gpuComparison.meetsMin && cpuComparison.user < gpuComparison.user * 0.7) {
      bottlenecks.push('Your CPU may bottleneck your GPU in this game.');
    }
    if (gpuComparison.meetsMin && cpuComparison.meetsMin && gpuComparison.user < cpuComparison.user * 0.7) {
    ...
    return {
      ...
      estimatedFPS,
      settings,
      resolution: systemScore >= 80 ? '1440p/4K' : systemScore >= 60 ? '1080p/1440p' : '1080p',
      componentAnalysis,
      bottlenecks,
  ...
  const renderDropdown = (type, list, placeholder, icon) => (
    <div className="relative">
      <select
        value={pcSpecs[type]}
        onChange={(e) => setPcSpecs(prev => ({ ...prev, [type]: e.target.value }))}
      ...
      >
        <option value="" className="bg-[#1a1a24] text-gray-500">Select {placeholder}</option>
        {list.map((item, index) => (
            ...
            <div className="p-6">
              {activeTab === 'details' ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-[#1a1a24] rounded-lg p-4">
                        ...
                        <ul className="space-y-2 text-xs">
                          ...
                          <li className="flex gap-2">
                            ...
                            <span className="text-gray-300">Intel Core i5-3570K</span>
                          </li>
                          <li className="flex gap-2">
                          ...
                          <li className="flex gap-2">
                            ...
                            <span className="text-gray-300">NVIDIA GTX 780</span>
                          </li>
                        </ul>
                      </div>
                      ...
                      <div className="bg-[#1a1a24] rounded-lg p-4">
                        ...
                        <ul className="space-y-2 text-xs">
                          ...
                          <li className="flex gap-2">
                            <span className="text-gray-500 w-16">CPU:</span>
                            <span className="text-gray-300">Intel Core i7-4790</span>
                          </li>
                          <li className="flex gap-2">
                            <span className="text-gray-500 w-16">RAM:</span>
                          ...
                          <li className="flex gap-2">
                            <span className="text-gray-500 w-16">GPU:</span>
                            <span className="text-gray-300">NVIDIA GTX 1060</span>
                          </li>
                        </ul>
                      </div>
                    </div>
                ...
                <div className="space-y-6">
                  <div>
                    <h3 className="text-sm font-medium text-[#9f7aea] mb-3 uppercase tracking-wider">Your PC Specs</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-4">
                        {renderDropdown('cpu', cpuList, 'CPU', <FaMicrochip className="text-sm" />)}
                        {renderDropdown('gpu', gpuList, 'GPU', <FaChartLine className="text-sm" />)}
                      </div>
                      <div className="space-y-4">
                        {renderDropdown('ram', ramList, 'RAM', <FaMemory className="text-sm" />)}
                        {renderDropdown('storage', storageList, 'Storage', <FaHdd className="text-sm" />)}
                      </div>
                    </div>
                    
                    <button
                      onClick={checkCompatibility}
                      disabled={checkingCompatibility || !pcSpecs.cpu || !pcSpecs.gpu || !pcSpecs.ram}
                  ...
                  {compatibilityResult && (
                    <div className="space-y-4 animate-in fade-in duration-500">
                      {/* Overall Result */}
                      <div className={`bg-[#1a1a24] rounded-lg p-6 border ${compatibilityResult.color} border-opacity-20`}>
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center justify-between mb-2">
                              <h4 className={`text-lg font-medium ${compatibilityResult.color}`}>
                                {compatibilityResult.compatibility}
                              </h4>
                              <span className={`text-3xl font-bold ${compatibilityResult.color}`}>
                                {compatibilityResult.systemScore}%
                              </span>
                            </div>
                            <p className="text-sm text-gray-300 mb-3">{compatibilityResult.message}</p>
                            
                            {/* Score Comparison Bar */}
                            <div className="relative h-2 bg-[#0f0f14] rounded-full overflow-hidden">
                              <div 
                                className="absolute h-full bg-[#9f7aea] rounded-full"
                                style={{ width: `${compatibilityResult.systemScore}%` }}
                              />
                              <div 
                                className="absolute h-full w-0.5 bg-white"
                                style={{ left: `${compatibilityResult.gameReqScore}%` }}
                              />
                            </div>
                            <div className="flex justify-between text-xs text-gray-500 mt-1">
                              <span>Your PC: {compatibilityResult.systemScore}%</span>
                              <span>Required: {compatibilityResult.gameReqScore}%</span>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-[#1a1a24] rounded-lg p-4">
                          <p className="text-xs text-gray-500 mb-1">Estimated FPS</p>
                          <p className={`text-sm font-medium ${compatibilityResult.color}`}>
                        ...
                        <div className="bg-[#1a1a24] rounded-lg p-4">
                          <p className="text-xs text-gray-500 mb-1">Performance Gap</p>
                          <p className={`text-sm font-medium ${compatibilityResult.difference >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                            {compatibilityResult.difference > 0 ? '+' : ''}{compatibilityResult.difference}%
                          </p>
                        </div>
                      </div>
                      ...
                      {/* Component Analysis */}
                      <div className="bg-[#1a1a24] rounded-lg p-4">
                        <h4 className="text-xs font-medium text-[#9f7aea] mb-3 uppercase tracking-wider">Component Analysis</h4>
                        <div className="space-y-4">
                          {compatibilityResult.componentAnalysis.map((component, index) => (
                            <div key={index}>
                              <div className="flex items-center justify-between mb-1">
                                <div className="flex items-center gap-2">
                                  <span className="text-sm text-gray-300 w-16">{component.name}</span>
                                  <span className="text-xs text-gray-500 truncate max-w-[150px]" title={component.userComponent}>
                                    {component.userComponent.length > 20 ? component.userComponent.substring(0, 20) + '...' : component.userComponent}
                                  </span>
                                </div>
                                <div className="flex items-center gap-3">
                                  <span className={`text-xs ${component.color}`}>{component.status}</span>
                                  <span className="text-xs text-gray-500">{component.userScore} pts</span>
                                </div>
                              </div>
                              <div className="flex items-center gap-2">
                                <div className="flex-1 h-2 bg-[#0f0f14] rounded-full overflow-hidden">
                                  <div 
                                    className={`h-full ${
                                      component.userScore >= component.recReq ? 'bg-emerald-400' :
                                      component.userScore >= component.minReq ? 'bg-blue-400' : 'bg-rose-400'
                                    }`}
                                    style={{ width: `${Math.min(100, component.userScore)}%` }}
                                  />
                                </div>
                                <div className="flex gap-2 text-[10px] text-gray-500">
                                  <span>Min: {Math.round(component.minReq)}</span>
                                  <span>Rec: {Math.round(component.recReq)}</span>
                                </div>
                              </div>
                            </div>
                          ))}
                      ...
                      {compatibilityResult.bottlenecks.length > 0 && (
                        <div className="bg-[#1a1a24] rounded-lg p-4 border border-yellow-500/20">
                          <h4 className="text-xs font-medium text-yellow-400 mb-3 uppercase tracking-wider">Potential Issues</h4>
                          <div className="space-y-2">
                            {compatibilityResult.bottlenecks.map((bottleneck, index) => (
                              <div key={index} className="flex items-start gap-2">
                                <FaExclamationTriangle className="text-yellow-400 text-xs mt-0.5" />
                                <p className="text-xs text-gray-300">{bottleneck}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {!compatibilityResult.meetsMinimum && (
                        <div className="bg-[#1a1a24] rounded-lg p-4 border border-[#9f7aea]/20">
                          <h4 className="text-xs font-medium text-[#9f7aea] mb-3 uppercase tracking-wider">Upgrade Recommendations</h4>
                          <div className="space-y-2">
                              ...
                              .map((component, index) => (
                                <div key={index} className="flex items-start gap-2">
                                  ...
                                  <p className="text-xs text-gray-300">
                                    Your {component.name} ({component.userComponent}) is below minimum requirements.
                                    {component.name === 'GPU' && ' Consider an RTX 3060 or RX 6600 for 1080p gaming.'}
                                    {component.name === 'CPU' && ' Consider an i5-12400F or Ryzen 5 5600X.'}
                                    {component.name === 'RAM' && ' 16GB DDR4-3200 is the sweet spot for gaming.'}
                                  </p>
                                </div>
                              ))}
                          </div>
